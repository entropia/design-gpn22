...
hb_color_grad = list(hb_high_color.range_to(hb_low_color, int(hb_color_shift_len/2))) + list(hb_low_color.range_to(hb_high_color, int(hb_color_shift_len/2)))

for i in range(hb_frames):
	keyframes.append([i+hb_start, "set_segment_color", tuple(map(lambda x: round(x, 3), hb_color_grad[i % hb_color_shift_len].rgb))])
